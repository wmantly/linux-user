...
var validUsernameRegex = /^([a-z_][a-z0-9_-]{0,31})$/;
...
exports.removeUser = function (username, callback) {
  ...
  spawnWrapper("userdel", ["-rf", username], null, callback);
};
...
exports.setPassword = function (username, password, callback) {
  ...
  // Use chpasswd instead of writing to passwd's stdin. chpasswd is designed
  // for non-interactive password updates and accepts "user:password" lines.
  spawnWrapper(
    "chpasswd",
    [],
    username + ":" + password + "\n",
...
exports.addUserToGroup = function (username, groupname, callback) {
  ...
  spawnWrapper("usermod", ["-a", "-G", groupname, username], null, callback);
};
    ...
    exports.getUserInfo(user, function (error, info) {
      ...
      function nextStep(err) {
        ...
        if (index >= steps.length) {
          return fs.readFile(keyFile, "utf8", function (readErr, existing) {
            ...
            if (existing) {
              ...
              for (var i = 0; i < lines.length; i++) {
                if (lines[i].trim() === normalizedKey) {
                  return callback(null, true);
                }
              }
            }

            fs.appendFile(keyFile, key + "\n", function (err) {
              if (err) {
                return callback(err);
              }
              callback(null, true);
